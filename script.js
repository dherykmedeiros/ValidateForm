     ...
     function validateSenha() {
         ...
         const senhaRegex = /^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[$*&@#])[0-9a-zA-Z$*&@#]{6,}$/
         if (senhaValue === '') {
             setError(senha, 'A senha é obrigatória');
             return false;
         } else if (!senhaRegex.test(senhaValue)) {
             setError(senha, 'A senha deve ter pelo menos 6 caracteres, coloque pelo menos 1 caractere especial, pelo menos 1 letra maiuscula, coloque pelo menos 1 numero, não coloque caracteres iguais em sequencia ex: aa, 00');
             return false;
         } else {
             setSuccess(senha);
             return true;
